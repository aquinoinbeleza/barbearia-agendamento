...
import HISTORIA from "./data/historicalEvents.json";
import INTERNACIONAIS from "./data/internationalDays.json";
import CURIOSIDADES from "./data/curiosities.json";
import CIENCIA from "./data/scienceDiscoveries.json";
import TECNOLOGIA from "./data/technologyMilestones.json";
import ESPACO from "./data/spaceExploration.json";
import AMBIENTE from "./data/environmentalEvents.json";
...
// Sugestões de domínio de e-mail — autocompletar DENTRO da caixa (estilo Gmail).
const EMAIL_DOMINIOS = ["gmail.com","hotmail.com","outlook.com","yahoo.com.br","yahoo.com","icloud.com","live.com","bol.com.br","terra.com.br","uol.com.br"];
// Retorna o e-mail completo sugerido (ou "") com base no que foi digitado — só depois do @.
const completarEmail = (v) => {
  const s = String(v || "");
  const at = s.indexOf("@");
  if (at < 0) return "";                                  // só sugere depois do @
  const local = s.slice(0, at);
  if (!local) return "";
  const dom = s.slice(at + 1);
  const domL = dom.toLowerCase();
  if (domL.indexOf("@") > -1 || domL.indexOf(" ") > -1) return ""; // @ repetido ou espaço
  if (EMAIL_DOMINIOS.indexOf(domL) > -1) return "";       // já completou um domínio conhecido
  const match = EMAIL_DOMINIOS.find(d => d.indexOf(domL) === 0 && d.length > domL.length);
  if (!match) return "";
  return s + match.slice(dom.length);                     // texto digitado + sufixo que falta
};

// Data de nascimento: a pessoa DIGITA (dd/mm/aaaa, só números) ou toca no
// botãozinho 📅 e escolhe no calendário — os dois sincronizados no mesmo valor.
const isoParaBR = (iso) => { const m = String(iso || "").match(/^(\d{4})-(\d{2})-(\d{2})$/); return m ? `${m[3]}/${m[2]}/${m[1]}` : ""; };
const NascimentoInput = ({ value, onChange, T }) => {
  const [txt, setTxt] = useState(isoParaBR(value));
  useEffect(() => { setTxt(isoParaBR(value)); }, [value]);
  const digitar = (v) => {
    const dig = v.replace(/\D/g, "").slice(0, 8);
    let fmt = dig;
    if (dig.length > 4) fmt = `${dig.slice(0,2)}/${dig.slice(2,4)}/${dig.slice(4)}`;
    else if (dig.length > 2) fmt = `${dig.slice(0,2)}/${dig.slice(2)}`;
    setTxt(fmt);
    if (dig.length === 8) {
      const d = +dig.slice(0,2), m = +dig.slice(2,4), a = +dig.slice(4);
      const dt = new Date(a, m - 1, d);
      const ok = a >= 1900 && dt.getFullYear() === a && dt.getMonth() === m - 1 && dt.getDate() === d && dt <= new Date();
      if (ok) onChange(`${dig.slice(4)}-${dig.slice(2,4)}-${dig.slice(0,2)}`);
    } else if (dig.length === 0) onChange("");
  };
  const est = { padding:"14px 16px", fontSize:16, borderRadius:13, border:`1.5px solid ${T.line}`, background:T.card, fontFamily:T.sans, outline:"none", color:T.ink };
  return (
    <div style={{display:"flex", gap:8, marginTop:8}}>
      <input value={txt} onChange={(e)=>digitar(e.target.value)} type="text" inputMode="numeric" placeholder="dd/mm/aaaa" maxLength={10}
        style={{...est, flex:1, minWidth:0}}
        onFocus={(e)=>e.target.style.borderColor=T.brass}
        onBlur={(e)=>{ e.target.style.borderColor=T.line; setTxt(isoParaBR(value)); }}/>
      <div style={{position:"relative", width:54, flexShrink:0}}>
        <div aria-hidden="true" style={{...est, padding:0, width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20}}>📅</div>
        <input value={value || ""} onChange={(e)=>onChange(e.target.value)} type="date" max={hojeISO()} min="1900-01-01"
          aria-label="Escolher a data no calendário"
          style={{position:"absolute", inset:0, width:"100%", height:"100%", opacity:0, cursor:"pointer", colorScheme:T.name==="dark"?"dark":"light"}}/>
      </div>
    </div>
  );
};

// Campo de e-mail com sugestão "fantasma" (cinza) dentro da caixa.
// Aceita a sugestão com Tab, Enter, seta → (no fim) ou clicando por cima.
const EmailInput = ({ value, onChange, T, placeholder }) => {
  const ref = useRef(null);
  const v = String(value || "");
  const sug = completarEmail(v);
  const ghost = sug ? sug.slice(v.length) : "";
  const aceitar = () => { if (ghost) { onChange(v + ghost); if (ref.current) ref.current.focus(); } };
  return (
    <div style={{position:"relative", marginTop:8}}>
      <input ref={ref} value={v} onChange={(e)=>onChange(e.target.value)} type="email" inputMode="email"
        autoComplete="email" autoCapitalize="none" spellCheck={false} placeholder={placeholder}
        onKeyDown={(e)=>{
          if (!ghost) return;
          const noFim = e.target.selectionStart === v.length && e.target.selectionEnd === v.length;
          if (e.key === "Tab" || e.key === "Enter" || (e.key === "ArrowRight" && noFim)) { e.preventDefault(); aceitar(); }
        }}
        style={{width:"100%",padding:"14px 16px",fontSize:16,borderRadius:13,border:`1.5px solid ${T.line}`,background:T.card,fontFamily:T.sans,outline:"none",color:T.ink}}
        onFocus={(e)=>e.target.style.borderColor=T.brass} onBlur={(e)=>e.target.style.borderColor=T.line}/>
      {ghost && (
        <div aria-hidden="true" style={{position:"absolute",left:0,top:0,right:0,bottom:0,padding:"14px 16px",fontSize:16,fontFamily:T.sans,display:"flex",alignItems:"center",whiteSpace:"pre",overflow:"hidden",pointerEvents:"none",borderRadius:13}}>
          <span style={{visibility:"hidden"}}>{v}</span>
          <span onMouseDown={(e)=>{ e.preventDefault(); aceitar(); }}
            onTouchEnd={(e)=>{ e.preventDefault(); aceitar(); }}
            style={{color:T.muted,pointerEvents:"auto",cursor:"pointer",flex:1,alignSelf:"stretch",display:"flex",alignItems:"center"}}>{ghost}</span>
        </div>
      )}
    </div>
  );
...
// Sessão do cliente: ao atualizar a página continua logado por 24h
// (ou até tocar em "Sair / trocar de número"). Guardamos só o telefone —
// os dados sempre vêm frescos do backend ao restaurar.
const SESSAO_KEY = "aquino_sessao";
const SESSAO_HORAS = 24;
const lerSessao = () => {
  try {
    const s = JSON.parse(localStorage.getItem(SESSAO_KEY));
    if (s && s.tel && (Date.now() - (s.ts || 0)) < SESSAO_HORAS * 3600 * 1000) return s;
  } catch (e) {}
  return null;
};
const salvarSessao = (ddi, paisIso, tel) => { try { localStorage.setItem(SESSAO_KEY, JSON.stringify({ ddi, paisIso, tel, ts: Date.now() })); } catch (e) {} };
const limparSessao = () => { try { localStorage.removeItem(SESSAO_KEY); } catch (e) {} };
...
const STRINGS = {
  ...
  insp_historia:{ pt:"Aconteceu neste dia: ", en:"On this day: ", es:"Un día como hoy: ", fr:"Un jour comme aujourd'hui : " },
  insp_intl:    { pt:"Datas internacionais: ", en:"International days: ", es:"Días internacionales: ", fr:"Journées internationales : " },
  insp_curiosidade:{ pt:"Curiosidade do dia: ", en:"Fact of the day: ", es:"Curiosidad del día: ", fr:"Le saviez-vous : " },
  insp_ciencia: { pt:"Descoberta científica: ", en:"Science discovery: ", es:"Descubrimiento científico: ", fr:"Découverte scientifique : " },
  insp_tecnologia:{ pt:"Marco tecnológico: ", en:"Tech milestone: ", es:"Hito tecnológico: ", fr:"Étape technologique : " },
  insp_espaco:  { pt:"Exploração espacial: ", en:"Space exploration: ", es:"Exploración espacial: ", fr:"Exploration spatiale : " },
  insp_ambiente:{ pt:"Meio ambiente: ", en:"Environment: ", es:"Medio ambiente: ", fr:"Environnement : " },
  t1_consultar: { pt:"Sob consulta — fale com a barbearia", en:"On request — ask the barbershop", es:"Bajo consulta — habla con la barbería", fr:"Sur demande — contactez le salon" },
  t1_a_consultar:{ pt:"a consultar", en:"on request", es:"a consultar", fr:"sur demande" },
  email_lgpd:   { pt:"Usamos seu e-mail só para confirmações e convites de agenda.", en:"We use your email only for confirmations and calendar invites.", es:"Usamos tu correo solo para confirmaciones e invitaciones.", fr:"Votre e-mail sert uniquement aux confirmations et invitations." },
...
const ddMMde = (d) => `${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
const historiaDe = (d) => HISTORIA[ddMMde(d)] || [];
const internacionaisDe = (d) => INTERNACIONAIS[ddMMde(d)] || [];
const cienciaDe = (d) => CIENCIA[ddMMde(d)] || [];
const tecnologiaDe = (d) => TECNOLOGIA[ddMMde(d)] || [];
const espacoDe = (d) => ESPACO[ddMMde(d)] || [];
const ambienteDe = (d) => AMBIENTE[ddMMde(d)] || [];
const NIVEIS = [
...
// v13: prefere o nível POR FREQUÊNCIA calculado no backend (janela móvel) quando disponível;
// cai no cálculo local por visitas só no modo demonstração (sem backend).
const fidelidadeView = (cli, localVisitas) => {
  const b = cli && cli.fidelidade;
  if (b && b.nivel) {
    const ref = (Number(b.visitasJanela)||0) + (Number(b.faltamVisitas)||0);
    return { nivel:b.nivel, prox:b.proximoNivel||null, faltam:Number(b.faltamVisitas)||0,
      pct: b.proximoNivel ? Math.max(6, Math.min(100, Math.round(((Number(b.visitasJanela)||0)/(ref||1))*100))) : 100,
      visitas:Number(b.visitasJanela)||0, manterAte:b.manterAte||null, porFrequencia:true };
  }
  return fidelidade(localVisitas);
};
...
const InspiracaoCard = ({ fraseIdx }) => {
  ...
  const intl = internacionaisDe(hoje);
  const hist = historiaDe(hoje);
  const ciencia = cienciaDe(hoje);
  const tecnologia = tecnologiaDe(hoje);
  const espaco = espacoDe(hoje);
  const ambiente = ambienteDe(hoje);
  const curi = CURIOSIDADES.length ? CURIOSIDADES[(hoje.getMonth()*31 + hoje.getDate()) % CURIOSIDADES.length] : "";
  ...
  return (
    <div style={{background:T.card,border:`1px solid ${T.line}`,borderRadius:16,padding:"16px"}}>
      ...
      {intl.length>0 && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_intl")}</span>{intl.join(" · ")}
        </div>
      )}
      {hist.length>0 && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_historia")}</span>{hist.join(" · ")}
        </div>
      )}
      {ciencia.length>0 && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_ciencia")}</span>{ciencia.join(" · ")}
        </div>
      )}
      {tecnologia.length>0 && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_tecnologia")}</span>{tecnologia.join(" · ")}
        </div>
      )}
      {espaco.length>0 && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_espaco")}</span>{espaco.join(" · ")}
        </div>
      )}
      {ambiente.length>0 && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_ambiente")}</span>{ambiente.join(" · ")}
        </div>
      )}
      {curi && (
        <div style={{marginTop:8,color:T.ink2,fontSize:12.5,lineHeight:1.5}}>
          <span style={{color:T.muted}}>{traduzir(idioma,"insp_curiosidade")}</span>{curi}
        </div>
      )}
...
// C5: documentos legais REAIS publicados em /public/legal (servidos pelo Vite) e
// renderizados aqui. Antes o modal mostrava só um resumo fixo, sem CNPJ/DPO e sem
// Consentimento/Cancelamento/Cookies — os 5 .md existiam mas não chegavam ao cliente.
const LEGAL_DOCS = [
  { key:"privacidade",   titulo:"Privacidade",   file:"01-Politica-de-Privacidade.md" },
  { key:"termos",        titulo:"Termos",        file:"02-Termos-de-Uso.md" },
  { key:"consentimento", titulo:"Consentimento", file:"03-Termo-de-Consentimento-LGPD.md" },
  { key:"cancelamento",  titulo:"Cancelamento",  file:"04-Politica-de-Cancelamento-e-NoShow.md" },
  { key:"cookies",       titulo:"Cookies",       file:"05-Aviso-de-Cookies.md" },
];
const LegalModal = ({ tipo, onClose }) => {
  const T = useT();
  const [docKey, setDocKey] = useState(LEGAL_DOCS.some(d=>d.key===tipo) ? tipo : "privacidade");
  const [md, setMd] = useState(null); // null=carregando · ""=falhou · texto=ok
  const info = LEGAL_DOCS.find(d=>d.key===docKey) || LEGAL_DOCS[0];
  useEffect(() => {
    let vivo = true; setMd(null);
    fetch(`/legal/${info.file}`).then(r => r.ok ? r.text() : Promise.reject()).then(txt => { if(vivo) setMd(txt); }).catch(() => { if(vivo) setMd(""); });
    return () => { vivo = false; };
  }, [docKey]); // eslint-disable-line
  const h2 = { fontFamily:T.serif, fontSize:17, fontWeight:700, color:T.ink, margin:"16px 0 6px" };
  const h3 = { fontFamily:T.sans, fontSize:14, fontWeight:700, color:T.ink, margin:"14px 0 4px" };
  const inline = (s) => String(s).split(/(\*\*[^*]+\*\*)/g).map((seg,i)=> /^\*\*[^*]+\*\*$/.test(seg) ? <b key={i}>{seg.slice(2,-2)}</b> : <span key={i}>{seg}</span>);
  const renderMd = (src) => {
    const linhas = String(src).split(/\r?\n/); const out = []; let buff = [];
    const flush = () => { if(buff.length){ out.push(<ul key={"ul"+out.length} style={{margin:"4px 0 10px 18px",padding:0}}>{buff}</ul>); buff=[]; } };
    linhas.forEach((ln, i) => {
      const l = ln.replace(/\s+$/,"");
      if(!l.trim()){ flush(); return; }
      if(/^###\s+/.test(l)){ flush(); out.push(<div key={i} style={{...h3,fontSize:13}}>{inline(l.replace(/^###\s+/,""))}</div>); }
      else if(/^##\s+/.test(l)){ flush(); out.push(<div key={i} style={h3}>{inline(l.replace(/^##\s+/,""))}</div>); }
      else if(/^#\s+/.test(l)){ flush(); out.push(<div key={i} style={h2}>{inline(l.replace(/^#\s+/,""))}</div>); }
      else if(/^[-*]\s+/.test(l)){ buff.push(<li key={i} style={{margin:"2px 0"}}>{inline(l.replace(/^[-*]\s+/,""))}</li>); }
      else if(/^---+$/.test(l.trim())){ flush(); out.push(<hr key={i} style={{border:"none",borderTop:`1px solid ${T.line}`,margin:"12px 0"}}/>); }
      else { flush(); out.push(<p key={i} style={{margin:"0 0 8px"}}>{inline(l)}</p>); }
    });
    flush(); return out;
  };
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:9999,backdropFilter:"blur(2px)"}}>
      <div onClick={(e)=>e.stopPropagation()} style={{background:T.bg,width:"100%",maxWidth:480,maxHeight:"86vh",borderRadius:"18px 18px 0 0",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 -10px 40px rgba(0,0,0,.4)"}}>
        <div style={{padding:"16px 22px 10px",borderBottom:`1px solid ${T.line}`}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <span style={{fontFamily:T.serif,fontSize:20,fontWeight:700,color:T.ink}}>{info.titulo}</span>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}}>
            {LEGAL_DOCS.map(d => (
              <button key={d.key} onClick={()=>setDocKey(d.key)} style={{padding:"5px 9px",borderRadius:8,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:T.sans,border:`1px solid ${docKey===d.key?T.brass:T.line}`,background:docKey===d.key?T.brassTint:"transparent",color:docKey===d.key?T.brass:T.muted}}>{d.titulo}</button>
            ))}
          </div>
        </div>
        <div style={{padding:"16px 22px",overflowY:"auto",fontSize:13.5,lineHeight:1.6,color:T.ink2,fontFamily:T.sans}}>
          <p style={{margin:"0 0 12px",fontSize:11.5,color:T.muted}}>AQUINO Barbearia &amp; Estética · CNPJ 34.828.065/0001-41 · R. Carlos Gomes, 256, Ideal, Ipatinga/MG · Encarregado de dados (DPO): Vinícius Julio de Aquino — aquino.inbeleza@gmail.com</p>
          {md === null ? <p style={{color:T.muted}}>Carregando documento…</p>
            : md === "" ? <p style={{color:T.muted}}>O documento completo está disponível em <b>/legal/{info.file}</b>. Em resumo: coletamos apenas os dados necessários para agendar e atender, conforme a LGPD (Lei nº 13.709/2018); você pode pedir acesso, correção ou exclusão e recusar mensagens promocionais respondendo <b>SAIR</b> no WhatsApp.</p>
            : renderMd(md)}
        </div>
...
function Portal() {
  ...
  const [servicosConsultar, setServicosConsultar] = useState([]); // (*) visíveis, sem clique
  ...
  const [bizDesc, setBizDesc] = useState(null); // config de desconto por múltiplos serviços (espelho do backend)
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const todosServ = rs && rs.servicos ? rs.servicos : DEMO_SERVICOS;
        setServicos(todosServ.filter(s=>s.ativo!==false));
        // VALORES.txt: serviços (*) ficam VISÍVEIS como "a consultar", sem clique
        setServicosConsultar(todosServ.filter(s=>s.ativo===false));
        setBarbeiros(rb && rb.barbeiros ? rb.barbeiros.filter(b=>b.ativo!==false) : DEMO_BARBEIROS);
        if (rc && rc.config) { if (rc.config.barbearia) setBizCfg(rc.config.barbearia); if (rc.config.operacao) setBizOp(rc.config.operacao); if (rc.config.descontos) setBizDesc(rc.config.descontos); }
      } catch (e) { setServicos(DEMO_SERVICOS); setBarbeiros(DEMO_BARBEIROS); }
    })();
  }, []);

  // sessão de 24h: ao atualizar a página, volta logado direto na Área do Cliente
  useEffect(() => {
    const s = lerSessao();
    if (!s || !ENV.hasBackend) return;
    setDdi(s.ddi || "55");
    if (s.paisIso) setPaisIso(s.paisIso);
    setTel(s.tel);
    (async () => {
      setVerificando(true);
      try {
        const limpo = numeroFinal(s.ddi || "55", s.tel);
        const r = await api.verificarCliente(limpo);
        if (r && r.encontrado && !r.bloqueado) {
          const [pn, sn] = dividirNome(r.nome || "");
          setClienteExistente(r); setNome(pn); setSobrenome(sn);
          setEmail(r.email || ""); setNascimento(nascParaInput(r.nascimento));
          setDependentes(depsParaEstado(r.dependentes || [])); setFotoUrl(r.foto || "");
          salvarSessao(s.ddi || "55", s.paisIso || "BR", s.tel); // renova as 24h
          setStep(HOME);
          carregarMeus(limpo);
        } else if (r && (r.encontrado === false || r.bloqueado)) {
          limparSessao(); // cadastro sumiu/bloqueado → volta ao início
        }
      } catch (e) {} // sem rede: fica na tela inicial com o número preenchido
  ...
  const avancarTelefone = async () => {
    ...
    try {
      ...
      if (r && r.encontrado) {
        ...
        setVerificando(false);
        salvarSessao(ddi, paisIso, tel); // mantém logado por 24h (até "Sair")
  ...
  // Desconto por múltiplos serviços — ESPELHO do cálculo do servidor (o backend recalcula e é a
  // fonte da verdade; aqui só mostramos a economia ao cliente). bizDesc vem do getConfig.
  const _dEhVar = (s)=>/a partir de|sob consulta|consultar/i.test(String((s&&s.nome)||""));
  const descPct = (()=>{ const dc=bizDesc||{}; if(dc.ativo===false) return 0; const eleg=(dc.excluirVariaveis!==false)?servArr.filter(s=>!_dEhVar(s)):servArr; const esc=[...(dc.escada||[])].sort((a,b)=>(Number(a.qtd)||0)-(Number(b.qtd)||0)); let p=0; esc.forEach(r=>{ if(eleg.length>=(Number(r.qtd)||0)) p=Number(r.pct)||0; }); const teto=Number(dc.teto); if(!isNaN(teto)&&teto>0) p=Math.min(p,teto); return Math.max(0,p); })();
  const _servBaseEleg = (bizDesc&&bizDesc.excluirVariaveis!==false)?servArr.filter(s=>!_dEhVar(s)).reduce((t,s)=>t+(Number(s.preco)||0),0):servTotalPreco;
  const servTotalComDesc = Math.round((_servBaseEleg*(100-descPct)/100 + (servTotalPreco-_servBaseEleg))*100)/100;
  const servEconomia = Math.round((servTotalPreco-servTotalComDesc)*100)/100;
  const mostrarDesc = (!bizDesc || bizDesc.mostrarCliente!==false) && descPct>0 && servEconomia>0;
  ...
  const confirmar = async () => {
    ...
    try {
      const r = await api.agendar({
        ...
        servicos: servArr.map(s => ({ id: s.id, nome: s.nome, duracao: s.duracao, preco: s.preco })), // id p/ o backend resolver preço/duração no config (C1)
        barbeiro: barbSel ? barbSel.nome : "", observacao: obs.trim(),
      });
      if (r && r._demo) { setResultado({ demo:true }); setStep(6); }
      else if (r && r.requiresSinal) { setResultado(r); salvarSessao(ddi, paisIso, tel); setStep(5); }
      else if (r && (r.success || r.id)) { setResultado(r); salvarSessao(ddi, paisIso, tel); carregarMeus(numeroFinal(ddi, tel)); setStep(6); }
  ...
  const resetTudo = () => {
    limparSessao(); // "Sair / trocar de número" encerra a sessão de 24h
    ...
    <Shell onToggleTema={onToggleTema}>
      ...
      <Bottom>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",marginBottom:12}}>
          {[["consentimento","Consentimento LGPD"],["cancelamento","Cancelamento e No-Show"],["cookies","Cookies"]].map(([k,lbl])=>(
            <button key={k} type="button" onClick={(e)=>{e.preventDefault();setLegalModal(k);}}
              style={{background:"none",border:"none",padding:0,color:T.muted,textDecoration:"underline",cursor:"pointer",fontSize:11,fontFamily:T.sans}}>{lbl}</button>
          ))}
        </div>
  ...
  if (step===HOME) {
    const fid = fidelidadeView(clienteExistente, visitasSeguras(clienteExistente, meusAgs.length));
    return (
      <Shell onToggleTema={onToggleTema}>
        ...
        <div style={{padding:"12px 22px 0"}}>
          <div style={{background:T.card,border:`1px solid ${T.line}`,borderRadius:16,padding:"14px 16px"}}>
            ...
            <div style={{color:T.muted,fontSize:11,marginTop:6}}>
              ...
              {fid.porFrequencia && fid.manterAte ? ` · volte até ${fid.manterAte} pra manter` : ""}
            </div>
          </div>
        </div>
      ...
      <Shell onToggleTema={onToggleTema}>
        ...
        <div style={{padding:"4px 22px 0"}}>
          ...
          <div style={{marginTop:14}}>
            ...
            <NascimentoInput value={nascimento} onChange={setNascimento} T={T}/>
          </div>
          <div style={{marginTop:14}}>
            <label style={{fontSize:13,fontWeight:600,color:T.ink2}}>{t("t4_email")}</label>
            <EmailInput value={email} onChange={setEmail} T={T} placeholder="seu@email.com" />
            <div style={{fontSize:11,color:T.muted,marginTop:6}}>{t("email_lgpd")}</div>
          </div>
  ...
  if (step===1) return (
    <Shell step={0} total={5} onToggleTema={onToggleTema}>
      ...
      {servicosConsultar.length>0 && (
        <div style={{padding:"18px 22px 0"}}>
          <div style={{fontSize:11,letterSpacing:"0.08em",textTransform:"uppercase",color:T.muted,marginBottom:8}}>{t("t1_consultar")}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {servicosConsultar.map(sv=>(
              <div key={sv.id} aria-disabled="true" style={{padding:"14px",borderRadius:14,background:T.card,border:`1.5px dashed ${T.line}`,opacity:.55,cursor:"not-allowed"}}>
                <div style={{fontWeight:700,fontSize:14,lineHeight:1.25,color:T.ink}}>{String(sv.nome).replace(/\s*\(sob consulta\)/i,"")}</div>
                <div style={{color:T.muted,fontSize:12,marginTop:6}}>{sv.duracao} min</div>
                <div style={{color:T.muted,fontWeight:700,fontSize:13,marginTop:2,fontStyle:"italic"}}>{t("t1_a_consultar")}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Bottom comBarra>
        ...
        {mostrarDesc && <div style={{textAlign:"center",fontSize:12,color:T.brass,marginBottom:8,fontWeight:700}}>🎉 Você economiza {money(servEconomia)} ({descPct}%) somando serviços</div>}
        <Primary onClick={()=>setStep(2)} disabled={!servArr.length}>{servArr.length?`${t("continuar")} · ${money(servTotalComDesc)}`:t("t1_selecione")}</Primary>
      </Bottom>
      <div style={{height:80}}/>
    ...
    <Shell step={3} total={5} onToggleTema={onToggleTema}>
      ...
      <div style={{padding:"4px 22px 0"}}>
        <div style={{background:T.card,border:`1px solid ${T.line}`,borderRadius:16,padding:18}}>
          ...
          {mostrarDesc && <Linha label={`Desconto (${descPct}%)`} valor={`− ${money(servEconomia)}`}/>}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:12,paddingTop:12,borderTop:`1px dashed ${T.line}`}}>
            ...
            <span style={{display:"flex",alignItems:"baseline",gap:8}}>
              {mostrarDesc && <span style={{fontSize:14,color:T.muted,textDecoration:"line-through"}}>{money(servTotalPreco)}</span>}
              <span style={{fontFamily:T.serif,fontWeight:700,fontSize:22,color:T.brass}}>{money(servTotalComDesc)}</span>
            </span>
          </div>
        </div>
