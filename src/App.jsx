...
const api = {
  ...
  // B1: o cancelamento do painel vai como admin (origem+key) → o backend permite ao DONO
  // cancelar mesmo dentro da janela de 12h (a regra continua valendo para o cliente).
  cancelar: (agendamentoId, tel) => api._post({ action: "cancelar", agendamentoId, tel, origem: "admin", key: adminKeyStore.get() }),
  // A4: registro manual de presença/falta pelo painel (admin) — permite marcar no-show
  marcarPresenca: (key, agendamentoId) => api._post({ action: "marcarPresenca", key, agendamentoId }),
  marcarFalta:    (key, agendamentoId) => api._post({ action: "marcarFalta", key, agendamentoId }),
  ...
  // v10: exclui o cliente do cadastro (agendamentos/financeiro ficam como histórico)
  clienteExcluir: (key, tel) => api._post({ action: "clienteExcluir", key, tel }),
  ...
  comandaFechar: (key, id, formaPagamento, cupom, gorjeta, fiado) => api._post({ action: "comandaFechar", key, id, formaPagamento, cupom, gorjeta, fiado }),
  fiadoListar: (key) => api._post({ action: "fiadoListar", key }),
  fiadoQuitar: (key, comandaId, formaPagamento) => api._post({ action: "fiadoQuitar", key, comandaId, formaPagamento }),
  ...
  metasPainel: (key, mes) => api._post({ action: "metasPainel", key, mes }),
  resumoMensal: (key, mes) => api._post({ action: "resumoMensal", key, mes }),
...
const DEFAULT_CONFIG = {
  ...
  // ── Descontos por múltiplos serviços ── escada % editável; o portal mostra a economia ao cliente.
  // % derivadas da média dos combos reais do AQUINO (2 svc ~10-13%, 3 svc ~15-17%).
  descontos: {
    ativo: true,
    mostrarCliente: true,
    excluirVariaveis: true,  // serviços "a partir de"/"sob consulta" não contam nem recebem desconto
    teto: 20,                // % máximo
    escada: [ { qtd: 2, pct: 10 }, { qtd: 3, pct: 15 }, { qtd: 4, pct: 18 } ],
  },
  // ── Metas de vendas da equipe ── meta da loja + por barbeiro, níveis e ranking no painel.
  metas: {
    ativo: true,
    metaLojaMes: 0,        // R$ — meta da loja no mês (0 = sem meta da loja)
    metaBarbeiroMes: 0,    // R$ — meta padrão por barbeiro (se o barbeiro não tiver a própria)
  ...
  // ── Fidelidade do cliente POR FREQUÊNCIA (janela móvel) ──
  // O nível mede a rotina nos últimos `janelaDias` (não o total acumulado): sobe quem mantém a
  // frequência e CAI quem começa a faltar. `intervaloDias` = rotina alvo de cada nível.
  fidelidade: {
    ativo: true,                 // liga/desliga o programa inteiro (oculta do cliente)
    rebaixamentoAtivo: true,     // liga/desliga a queda de nível
    janelaDias: 90,              // janela móvel de medição
    quedaSumicoDias: 30,         // sumiu mais que isso sem vir → cai 1 nível
    niveis: [
      { id:3, label:"Ouro",   icon:"✦", cor:"#C18A3D", intervaloDias:7,  min:12, beneficios:["Fila VIP","Bônus 5%","Desconto na barba"] },
      { id:2, label:"Prata",  icon:"◆", cor:"#9aa0a6", intervaloDias:15, min:6,  beneficios:["Desconto 5%","Bônus mensal"] },
      { id:1, label:"Bronze", icon:"○", cor:"#B0814F", intervaloDias:30, min:0,  beneficios:["Programa básico"] },
    ],
    rebaixamento: { faltas: 1, dias: 15 }, // legado (fallback do cálculo antigo por visitas)
    recompensas: [
  ...
  // ── Sorteio de prêmios por avaliação (BarberMeta) ── começa DESLIGADO; some prêmios e pesos.
  sorteio: {
    ativo: false,
    premios: [
  ...
  // ── Central do Projeto (aba DEMO/template) ── checklist vivo: o que já está pronto, o que
  // falta pra lançar e o backlog. Editável e salvo — serve de MODELO para projetos futuros.
  projeto: {
    versao: "v15",
    modoDemo: true,            // marca este build como "versão de criação / template"
    prontos: [
      "Agendamento online multi-serviço", "Desconto por múltiplos serviços", "Portal logado 24h",
      "Documentos legais publicados (LGPD)", "Anti-fraude de preço (servidor)", "Bot WhatsApp (menu/NPS/lembretes)",
      "4+ estrelas → Google Reviews", "Caixa / Comanda / Cupons / Pontos", "Comissão por barbeiro",
      "Metas de venda + ranking + link do barbeiro", "Níveis do cliente por frequência", "Sorteio de prêmios",
      "Resumo mensal", "Fila de espera", "Bloqueios de horário", "PWA (instalável)", "Telefones mascarados (LGPD)",
      "Gorjeta na comanda", "Fiado / crédito do cliente", "Controle de estoque de produtos",
    ],
    lancamento: [
      { id:"backend",  label:"Publicar o backend (Apps Script → Nova versão)", feito:false },
      { id:"frontend", label:"Publicar o frontend (GitHub → Vercel)", feito:false },
      { id:"seed",     label:"Rodar seedDadosReais()", feito:false },
      { id:"triggers", label:"Rodar criarTriggers() (9 gatilhos)", feito:false },
      { id:"senha",    label:"Trocar senha do Google + ativar 2FA", feito:false },
      { id:"tokens",   label:"Trocar ADMIN_KEY + SITE_TOKEN + VERIFY_TOKEN", feito:false },
      { id:"apagar",   label:"Apagar IMPLANTAÇÃO TOTAL.txt + SITE.zip (tinham a senha)", feito:false },
      { id:"whatsapp", label:"WhatsApp Meta: 6 templates aprovados + aquecer o número", feito:false },
      { id:"calendar", label:"Vincular CALENDAR_ID dedicado (aba Google Agenda)", feito:false },
      { id:"mp",       label:"Mercado Pago: MP_ACCESS_TOKEN + GAS_WEBHOOK_URL (se cobrar sinal)", feito:false },
      { id:"props",    label:"Properties: SITE_URL, SAC_NUMERO, GOOGLE_MAPS_LINK", feito:false },
      { id:"smoke",    label:"Rodar os 8 smoke tests em aba anônima", feito:false },
      { id:"logo",     label:"Definir logo + link de avaliação no Google", feito:false },
    ],
    backlog: [
      { id:"mes",         label:"Agenda: visão Mês + arrastar-e-soltar", feito:false },
      { id:"recorrencia", label:"Recorrência automática de agendamento", feito:false },
      { id:"rbac",        label:"Esconder menus por perfil (RBAC na tela)", feito:false },
      { id:"financeiro",  label:"Gráfico do Financeiro com dados reais", feito:false },
      { id:"marcos",      label:"Marcos (5ª/10ª/20ª) por comparecimento, não por agendamento", feito:false },
      { id:"sorteiocup",  label:"Sorteio → gerar cupom automático", feito:false },
      { id:"pollsinal",   label:"Portal: confirmar pagamento do sinal ao vivo (polling)", feito:false },
      { id:"duplicatas",  label:"Mesclar clientes duplicados", feito:false },
      { id:"cpf",         label:"Ficha: CPF + 'Indicado por' (indicação)", feito:false },
      { id:"anamnese",    label:"Anamnese com assinatura", feito:false },
      { id:"sumido45",    label:"Fluxo 'cliente sumido 45+ dias'", feito:false },
      { id:"botbotoes",   label:"Bot com menu de botões (Meta interativo)", feito:false },
      { id:"otp",         label:"OTP na área do cliente (proteger dados)", feito:false },
      { id:"giftcard",    label:"Gift card / clube de assinatura / cashback", feito:false },
      { id:"happyhour",   label:"Happy hour / desconto em horário ocioso", feito:false },
      { id:"precoprof",   label:"Preço e duração do serviço por profissional", feito:false },
      { id:"gaptime",     label:"Folga automática entre atendimentos (gap time)", feito:false },
      { id:"totem",       label:"Totem de autoatendimento (check-in na chegada)", feito:false },
      { id:"noshow",      label:"Cobrança automática de no-show (falta)", feito:false },
      { id:"optout",      label:"Opt-out granular (bloquear só mensagens automáticas)", feito:false },
      { id:"flyer",       label:"Gerador de posts para redes (digital flyer)", feito:false },
      { id:"editorprog",  label:"Editor de agendamento progressivo no botão +", feito:false },
      { id:"automacoes",  label:"Automações configuráveis (ligar/desligar disparos + horários)", feito:false },
      { id:"recursos",    label:"Recursos/salas (cadeiras) para a estética", feito:false },
      { id:"antecip",     label:"Antecipação de recebíveis (V3)", feito:false },
      { id:"push",        label:"Web Push remoto (precisa VAPID + provedor)", feito:false },
      { id:"nfe",         label:"NF-e (sem automação grátis p/ MEI)", feito:false },
      { id:"multi",       label:"Multiunidade (outra arquitetura)", feito:false },
    ],
  },
  // Produtos à venda (estoque) — editáveis no painel; baixa automática ao fechar a comanda.
  produtos: [],
};
...
// v11: identidade AQUINO no painel — preto + dourado (mesma elegância do portal
// do cliente). "blue"/"cyan" viram tons de latão/champanhe; os semânticos
// (verde=ok, âmbar=aviso, vermelho=erro) continuam universais.
const PAL_DARK = {
  bg0:"#0B0A08", bg1:"#11100C", bg2:"#171511", bg3:"#1E1B15", bg4:"#28231A",
  border:"#262117", borderHi:"#3A3322",
  textPri:"#EDE8DF", textSec:"#A4977F", textMuted:"#5C5240",
  blue:"#C18A3D", blueHi:"#D8A55C",
  cyan:"#E0B775",
  green:"#4ADE80",
  amber:"#F59E0B", red:"#F87171", purple:"#A78BFA",
};
const PAL_LIGHT = {
  bg0:"#F4F1EA", bg1:"#FFFFFF", bg2:"#FAF7F1", bg3:"#F0EBE1", bg4:"#E4DCCB",
  border:"#E6DFD0", borderHi:"#CFC3AB",
  textPri:"#2A2418", textSec:"#6E6149", textMuted:"#A2937A",
  blue:"#A06E2A", blueHi:"#B8853F",
  cyan:"#8F6A28",
  green:"#16A34A",
...
const adaptDashboard = (r) => {
  const clientes = (r.clientes || []).map(c => ({
    ...
    fidelidade: c.fidelidade || null,       // nível por frequência (janela móvel) — p/ o badge na lista
  }));
...
const SEARCH_INDEX = [
  ...
  { type:"page", label:"Central",    sub:"Roadmap e versão DEMO",      nav:"central",   color:A.textSec, icon:"reports" },
];
...
const Sidebar = ({ active, setActive }) => {
  ...
  const nav = [
    ...
    { id:"waitlist", label:"Fila de espera", icon:"waitlist", badge:String(DB.waitlist.length) },
      ...
      <div style={{ padding:"20px 14px 16px", borderBottom:`1px solid ${A.border}` }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            ...
            <div style={{ color:A.textMuted, fontSize:8, letterSpacing:"0.1em", textTransform:"uppercase" }}>Barbearia & Estética</div>
          </div>
        </div>
      </div>
...
const AgendaPage = () => {
  ...
  const cfg = {
    realizado: {color:A.green,label:"Realizado"},
    presenca_confirmada:{color:A.green,label:"Presença"}, // C3: antes caía em "Vago" (cinza)
    confirmado:{color:A.cyan,label:"Confirmado"},
    aguardando_sinal:{color:A.amber,label:"Aguardando sinal"},
    aguardando:{color:A.amber,label:"Aguardando"},
    intervalo: {color:A.textMuted,label:"Intervalo"},
    risco:     {color:A.red,label:"Risco"},
    faltou:    {color:A.red,label:"Faltou"}, // A4: no-show registrado pelo dono
  ...
  // A4: registrar PRESENÇA / FALTA do cliente (resolve o gap operacional nº 1 — antes o
  // dono não tinha como marcar um no-show, e o verificarFeedback faturava todo confirmado).
  const handlePresenca = async (s) => {
    if (!ENV.hasBackend) { setAgendaState(prev=>prev.map(x=>x.id===s.id?{...x,status:"presenca_confirmada"}:x)); toast(`${s.nome} — presença confirmada (demo)`, A.green, "check"); return; }
    try {
      const r = await api.marcarPresenca(adminKeyStore.get(), s.id);
      if (r && r.success) { setAgendaState(prev=>prev.map(x=>x.id===s.id?{...x,status:"presenca_confirmada"}:x)); toast(`${s.nome} — presença confirmada ✓`, A.green, "check"); }
      else toast((r&&r.error)||"Não foi possível registrar presença", A.amber, "warning");
    } catch(e){ toast("Falha de rede ao registrar presença", A.red, "warning"); }
  };
  const handleFalta = async (s) => {
    if (!ENV.hasBackend) { setAgendaState(prev=>prev.map(x=>x.id===s.id?{...x,status:"faltou"}:x)); toast(`${s.nome} — marcado como falta (demo)`, A.amber, "warning"); return; }
    if (typeof window!=="undefined" && !window.confirm(`Marcar FALTA de ${s.nome} às ${s.hora}? Não conta como atendido.`)) return;
    try {
      const r = await api.marcarFalta(adminKeyStore.get(), s.id);
      if (r && r.success) { setAgendaState(prev=>prev.map(x=>x.id===s.id?{...x,status:"faltou"}:x)); toast(`${s.nome} — falta registrada`, A.amber, "warning"); }
      else toast((r&&r.error)||"Não foi possível registrar a falta", A.amber, "warning");
    } catch(e){ toast("Falha de rede ao registrar falta", A.red, "warning"); }
  };
  ...
  const handleNovoSubmit = async () => {
    ...
    try {
      const r = await api.agendarManual(adminKeyStore.get(), {
        ...
        servico: { id: sv.id, nome: sv.nome, duracao: sv.duracao, preco: sv.preco },
                  ...
                  <div key={i} style={{
                    ...
                    {!isCancelado&&s.nome!=="—"&&(
                      <div style={{display:"flex",gap:5}}>
                        ...
                        {s.status!=="realizado"&&s.status!=="presenca_confirmada"&&s.status!=="faltou"&&(
                          <>
                            <Btn variant="secondary" size="sm" onClick={()=>handlePresenca(s)}><Ico n="check" size={10} color={A.green}/>Presença</Btn>
                            <Btn variant="ghost" size="sm" onClick={()=>handleFalta(s)}>Faltou</Btn>
                          </>
                        )}
      ...
      <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",gap:S.md}}>
        <div style={{display:"flex",gap:S.md}}>
          {[{label:"Ativos",v:String(DB.clientes.length),c:A.blue},{label:"VIPs",v:DB.clientes.filter(c=>c.nivel==="VIP").length,c:A.cyan},
            {label:"Em risco",v:DB.clientes.filter(c=>BE.shouldFlagRisk(c)).length,c:A.red},{label:"Ticket médio",v:(DB.mes&&DB.mes.ticket)?`R$ ${Math.round(DB.mes.ticket)}`:"—",c:A.green}].map((st,i)=>(
            <Card key={i} style={{flex:1}}>
            ...
            {filtered.map((c,i)=>(
              ...
              >
                ...
                <div style={{flex:1,display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  {c.fidelidade&&c.fidelidade.nivel&&<span title="Nível por frequência (janela móvel)" style={{fontSize:9,padding:"1px 6px",borderRadius:999,border:`1px solid ${c.fidelidade.cor||A.textSec}`,color:c.fidelidade.cor||A.textSec}}>{c.fidelidade.nivel}</span>}
                </div>
      ...
      {sel&&(
        <div style={{width:270,flexShrink:0,display:"flex",flexDirection:"column",gap:S.md,animation:`slideIn ${M.base}`}}>
          ...
          {cliTab==="ficha"&&(<Card>
            ...
            <Btn variant="danger" size="sm" style={{width:"100%",justifyContent:"center",marginTop:S.sm}} onClick={async ()=>{
              if(!ENV.hasBackend){ toast("Exclusão disponível só com o backend conectado", A.amber, "warning"); return; }
              if(typeof window!=="undefined" && !window.confirm(`EXCLUIR ${sel.nome} do cadastro?\n\nIsso remove a ficha do cliente (nome, contato, pontos). O histórico de agendamentos e o financeiro são mantidos.\n\nEssa ação não pode ser desfeita.`)) return;
              const key = adminKeyStore.get() || (typeof window!=="undefined" ? window.prompt("Chave de admin (ADMIN_KEY do GAS):") : "");
              if(!key) return;
              try{
                const r = await api.clienteExcluir(key, sel.tel);
                if(r&&r.success){
                  toast(`${sel.nome} excluído do cadastro`, A.green, "check");
                  setSelected(null);
                  // recarrega o dashboard p/ a lista refletir a exclusão
                  api.dashboard(key).then(d=>{ if(d&&d.success&&d.autenticado) liveDataStore.set(adaptDashboard(d)); }).catch(()=>{});
                } else toast((r&&r.error)||"Não foi possível excluir", A.amber, "warning");
              }catch(e){ toast("Falha de rede ao excluir", A.red, "warning"); }
            }}>Excluir cliente</Btn>
          </Card>)}
...
const CaixaPage = () => {
  ...
  const [cmGorjeta, setCmGorjeta] = useState(""); const [cmFiado, setCmFiado] = useState(false); const [addProdId, setAddProdId] = useState(""); const [fiados, setFiados] = useState(null);
  ...
  const abrirComanda = (c) => { setCmAtual(c); setCmItens(c.itens||[]); setCmDesc(c.desconto||0); setCmCupom(""); setCmPag({ dinheiro:"", pix:"", credito:"", debito:"" }); setAddServId(""); setFreeNome(""); setFreePreco(""); setCmGorjeta(""); setCmFiado(false); setAddProdId(""); setCmOpen(true); };
  const addProduto = () => { const p=(conf.produtos||[]).find(x=>String(x.id)===String(addProdId)); if(!p) return; setCmItens(prev=>[...prev,{nome:p.nome,preco:Number(p.preco)||0,qtd:1,produtoId:p.id}]); setAddProdId(""); };
  const carregarFiados = async () => { if(!ENV.hasBackend){ toast("Conecte o backend p/ ver fiados", A.amber, "warning"); return; } const r = await api.fiadoListar(key()); if(r&&r.success) setFiados(r); };
  const quitarFiado = async (comandaId) => { const r = await api.fiadoQuitar(key(), comandaId, "dinheiro"); if(r&&r.success){ toast("Fiado quitado ✓", A.green, "check"); carregarFiados(); recarregar(); } else toast((r&&r.error)||"Não foi possível quitar", A.amber, "warning"); };
  ...
  const fecharComanda = async () => {
    ...
    let pag; if (cmFiado) { pag = "fiado"; } else { pag = {}; ["dinheiro","pix","credito","debito"].forEach(m=>{ const v=Number(cmPag[m])||0; if(v>0) pag[m]=v; }); }
    setBusy(true);
    await api.comandaAtualizar(key(), cmAtual.id, cmItens, Number(cmDesc)||0);
    const cupomUsado = (Number(cmDesc) > 0 && cmCupom) ? cmCupom.trim().toUpperCase() : "";
    const r = await api.comandaFechar(key(), cmAtual.id, pag, cupomUsado, Number(cmGorjeta)||0, cmFiado); setBusy(false);
    if (r && r.success) { toast(`Comanda ${cmFiado?"no fiado":"fechada"} · R$ ${Number(r.total||0).toFixed(2)}${r.gorjeta?` · gorjeta R$ ${Number(r.gorjeta).toFixed(2)}`:""}${r.pontosGanhos?` · +${r.pontosGanhos} pts`:""}`, A.green, "check"); setCmOpen(false); recarregar(); }
    else toast((r&&r.error)||"Não foi possível fechar", A.amber, "warning");
  };
    ...
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      ...
      {!caixa ? (
        ...
        <>
          ...
          <Card pad={false}>
            <div style={{padding:`${S.md}px ${S.xl}px`,borderBottom:`1px solid ${A.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{color:A.textPri,fontWeight:700,fontSize:13}}>Fiados em aberto{fiados?` · R$ ${Number(fiados.total||0).toFixed(2)}`:""}</div>
              <Btn variant="secondary" size="sm" onClick={carregarFiados} disabled={semBackend}>Atualizar</Btn>
            </div>
            <div style={{padding:`${S.sm}px ${S.md}px`}}>
              {!fiados && <div style={{color:A.textMuted,fontSize:11,padding:"10px 0",textAlign:"center"}}>Clique em Atualizar para ver os fiados em aberto.</div>}
              {fiados && (fiados.fiados||[]).length===0 && <div style={{color:A.textMuted,fontSize:11,padding:"10px 0",textAlign:"center"}}>Nenhum fiado em aberto.</div>}
              {fiados && (fiados.fiados||[]).map((f,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 8px",borderBottom:i<(fiados.fiados.length-1)?`1px solid ${A.border}`:"none"}}>
                  <div style={{flex:1,minWidth:0}}><div style={{color:A.textPri,fontSize:12}}>{f.descricao}</div><div style={{color:A.textMuted,fontSize:10}}>{f.data}</div></div>
                  <span style={{color:A.amber,fontSize:12,fontFamily:SHARED.fontMono}}>R$ {Number(f.valor||0).toFixed(2)}</span>
                  <Btn variant="secondary" size="sm" onClick={()=>quitarFiado(f.comandaId)}>Quitar</Btn>
                </div>
              ))}
            </div>
          </Card>
      ...
      {cmOpen && cmAtual && (<Modal onClose={()=>!busy&&setCmOpen(false)} width={520}>
        ...
        {(conf.produtos||[]).filter(p=>p.ativo!==false).length>0 && (
        <div style={{display:"flex",gap:6,alignItems:"flex-end",marginBottom:8}}>
          <div style={{flex:2}}><Field label="Produto (dá baixa no estoque)">
            <select value={addProdId} onChange={e=>setAddProdId(e.target.value)} style={selStyle}>
              <option value="">Escolha…</option>
              {(conf.produtos||[]).filter(p=>p.ativo!==false).map(p=>(<option key={p.id} value={p.id}>{p.nome} · R$ {p.preco} · {Number(p.estoque)||0} un</option>))}
            </select>
          </Field></div>
          <Btn variant="secondary" size="sm" onClick={addProduto}>Adicionar</Btn>
        </div>)}
        ...
        <div style={{marginBottom:10}}>
          ...
          <div style={{marginTop:6,fontSize:10,color:cmFiado?A.amber:(Math.abs(pagSoma-cmTotal)<0.01?A.green:A.amber)}}>{cmFiado?"No fiado — o pagamento fica em aberto até quitar.":`Pago: R$ ${pagSoma.toFixed(2)} · Falta: R$ ${Math.max(0,cmTotal-pagSoma).toFixed(2)}`}</div>
        </div>
        <div style={{display:"flex",gap:10,alignItems:"flex-end",marginBottom:10}}>
          <div style={{flex:1}}><Field label="Gorjeta (R$)"><TextInput type="number" value={cmGorjeta} onChange={setCmGorjeta} placeholder="0,00"/></Field></div>
          <label style={{flex:1,display:"flex",alignItems:"center",gap:8,cursor:"pointer",paddingBottom:6}}>
            <input type="checkbox" checked={cmFiado} onChange={e=>setCmFiado(e.target.checked)} style={{width:17,height:17,accentColor:A.amber}}/>
            <span style={{fontSize:12,color:A.textPri}}>Fechar no <b>fiado</b> (pagar depois)</span>
          </label>
        </div>
...
const LoyaltyPage = () => {
  ...
  const sorteio = (configStore.get() || {}).sorteio || DEFAULT_CONFIG.sorteio;
  const setSorteio = (patch) => configStore.set(c => ({ ...c, sorteio: { ...(c.sorteio || DEFAULT_CONFIG.sorteio), ...patch } }));
  // salva fidelidade/sorteio NO SERVIDOR (o store local não basta — o backend é a fonte do nível)
  const salvarFidBackend = async () => {
    const c = configStore.get();
    const payload = { barbearia:c.barbearia, servicos:c.servicos, barbeiros:c.barbeiros||[], horarios:c.horarios, operacao:c.operacao, fidelidade:c.fidelidade, descontos:c.descontos, metas:c.metas, sorteio:c.sorteio, projeto:c.projeto, produtos:c.produtos };
    if(!ENV.hasBackend){ toast("Salvo localmente (demonstração)", A.green, "check"); return; }
    const key = window.prompt("Chave de admin (ADMIN_KEY) para salvar no servidor:");
    if(!key){ toast("Salvamento cancelado", A.textSec, "warning"); return; }
    try { const r = await api.salvarConfig(key, payload); toast(r&&r.success?"Fidelidade salva no servidor ✓ (já vale pro cliente)":(r&&(r.error||r.erro))||"Chave recusada", r&&r.success?A.green:A.amber, r&&r.success?"check":"warning"); }
  ...
  const salvarNivel = (idx, campo, valor) => {
    const niveis = niveisConfig.map((n,i) => i===idx ? { ...n, [campo]: (campo==="min"||campo==="intervaloDias") ? Number(valor) : valor } : n);
    setFid({ niveis });
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      ...
      <div style={{display:"flex",gap:6}}>
        {[["niveis","🏆 Níveis"],["recompensas","🎁 Recompensas"],["sorteio","🎰 Sorteio"],["top","⭐ Top Clientes"]].map(([id,label])=>(
        ...
        <div style={{flex:1}}/>
        <button onClick={salvarFidBackend} style={{padding:"7px 16px",borderRadius:R.md,border:`1px solid ${A.green}`,
          background:`${A.green}15`,color:A.green,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:SHARED.fontSans}}>
          Salvar no servidor
        </button>
      </div>
      ...
      {aba==="niveis" && (
        <div style={{display:"flex",gap:S.md}}>
          <Card style={{flex:1}}>
            <SectionHead title="Configuração de Níveis" sub="Por frequência: a rotina (dias) de cada nível, medida numa janela móvel"/>
            <div style={{display:"flex",gap:12,flexWrap:"wrap",background:A.bg3,border:`1px solid ${A.border}`,borderRadius:R.md,padding:"10px 12px",marginBottom:S.sm}}>
              <div>
                <div style={{color:A.textMuted,fontSize:9,marginBottom:4}}>Janela de medição (dias)</div>
                <input type="number" value={fid.janelaDias||90} onChange={e=>setFid({janelaDias:Number(e.target.value)||90})}
                  style={{width:70,background:A.bg2,border:`1px solid ${A.border}`,borderRadius:R.sm,padding:"5px 8px",color:A.textPri,fontSize:12,fontFamily:SHARED.fontMono}}/>
              </div>
              <div>
                <div style={{color:A.textMuted,fontSize:9,marginBottom:4}}>Sumiu &gt; X dias → cai 1 nível</div>
                <input type="number" value={fid.quedaSumicoDias||30} onChange={e=>setFid({quedaSumicoDias:Number(e.target.value)||30})}
                  style={{width:70,background:A.bg2,border:`1px solid ${A.border}`,borderRadius:R.sm,padding:"5px 8px",color:A.textPri,fontSize:12,fontFamily:SHARED.fontMono}}/>
              </div>
              <div style={{flex:1,minWidth:160,alignSelf:"center",color:A.textSec,fontSize:10.5}}>
                Ex.: Ouro a cada 7 dias ≈ {Math.floor((fid.janelaDias||90)/7)} visitas na janela. Não conta histórico antigo — sobe quem mantém a rotina, cai quem some.
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:S.sm}}>
                  ...
                  <div key={i} style={{background:A.bg3,border:`1px solid ${n.cor}20`,borderRadius:R.lg,padding:"13px 15px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div>
                          ...
                          {!isEdit && <div style={{color:A.textMuted,fontSize:9}}>Rotina: vir a cada {n.intervaloDias||30} dias</div>}
                        </div>
                      </div>
                    ...
                    {isEdit && (
                      <div style={{background:A.bg2,borderRadius:R.md,padding:"12px",marginBottom:10,display:"flex",gap:12,flexWrap:"wrap"}}>
                        <div>
                          <div style={{color:A.textMuted,fontSize:9,marginBottom:4}}>Rotina: vir a cada (dias)</div>
                          <input type="number" value={n.intervaloDias||30} onChange={e=>salvarNivel(i,"intervaloDias",e.target.value)}
      ...
      {/* ABA SORTEIO — prêmio por avaliação (BarberMeta) */}
      {aba==="sorteio" && (
        <Card>
          <SectionHead title="Sorteio de prêmios por avaliação" sub="Cliente avalia com 4-5 estrelas → sorteia um prêmio (peso = chance). Resgate no balcão."/>
          <label style={{display:"flex",alignItems:"center",gap:10,marginBottom:S.md}}>
            <div onClick={()=>setSorteio({ativo:!sorteio.ativo})} style={{cursor:"pointer",width:46,height:26,borderRadius:R.pill,position:"relative",background:sorteio.ativo?A.green:A.border,flexShrink:0}}>
              <div style={{position:"absolute",top:3,left:sorteio.ativo?23:3,width:20,height:20,borderRadius:"50%",background:"#fff"}}/>
            </div>
            <b style={{fontSize:13,color:A.textPri}}>Sorteio {sorteio.ativo?"ativo":"desligado"}</b>
          </label>
          {(sorteio.premios||[]).map((p,i)=>{
            const total=(sorteio.premios||[]).reduce((s,x)=>s+(Number(x.peso)||0),0)||1;
            return (
            <div key={i} style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
              <input value={p.nome} onChange={e=>setSorteio({premios:sorteio.premios.map((x,j)=>j===i?{...x,nome:e.target.value}:x)})} placeholder="Prêmio"
                style={{flex:1,background:A.bg3,border:`1px solid ${A.border}`,borderRadius:R.sm,padding:"6px 8px",color:A.textPri,fontSize:12,fontFamily:SHARED.fontSans}}/>
              <input type="number" value={p.peso} onChange={e=>setSorteio({premios:sorteio.premios.map((x,j)=>j===i?{...x,peso:Number(e.target.value)||0}:x)})} title="Peso (chance)"
                style={{width:60,background:A.bg3,border:`1px solid ${A.border}`,borderRadius:R.sm,padding:"6px 8px",color:A.textPri,fontSize:12,fontFamily:SHARED.fontMono}}/>
              <span style={{width:42,fontSize:10.5,color:A.textMuted,textAlign:"right"}}>{Math.round((Number(p.peso)||0)/total*100)}%</span>
              <button onClick={()=>setSorteio({premios:sorteio.premios.filter((_,j)=>j!==i)})} style={{padding:"6px 9px",borderRadius:R.sm,border:`1px solid ${A.border}`,background:A.bg2,color:A.textSec,fontSize:11,cursor:"pointer"}}>✕</button>
            </div>
          );})}
          <button onClick={()=>setSorteio({premios:[...(sorteio.premios||[]),{nome:"Novo prêmio",peso:10}]})} style={{padding:"6px 12px",borderRadius:R.sm,border:`1px solid ${A.border}`,background:A.bg2,color:A.textSec,fontSize:11,cursor:"pointer",marginTop:4}}>+ Adicionar prêmio</button>
          <div style={{marginTop:S.md,fontSize:10.5,color:A.textMuted}}>O cliente recebe a mensagem do prêmio no WhatsApp; o resgate é manual no balcão (você confere). Lembre de clicar em “Salvar no servidor”.</div>
        </Card>
      )}
...
const WaitlistPage = () => {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      <div style={{display:"flex",gap:S.md}}>
        {[{l:"Na fila agora",v:fila.length,c:A.purple},{l:"TTL ativo",v:"—",c:A.amber},
          {l:"Convertidos hoje",v:"—",c:A.green},{l:"Taxa conversão",v:"—",c:A.cyan}].map((st,i)=>(
          <Card key={i} style={{flex:1}}>
            <div style={{color:A.textMuted,fontSize:9,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:4}}>{st.l}</div>
            <div style={{color:st.c,fontSize:22,fontWeight:800,fontFamily:SHARED.fontMono}}>{st.v}</div>
...
const RelatoriosPage = () => {
  ...
  // v13: resumo mensal (BarberMeta) — números do mês + texto pronto p/ o dono
  const [resumo, setResumo] = useState(null);
  useEffect(() => {
    if (!ENV.hasBackend || !adminKey) { setResumo(null); return; }
    let vivo = true;
    api.resumoMensal(adminKey).then(r => { if (vivo && r && r.success) setResumo(r); }).catch(()=>{});
    return () => { vivo = false; };
  }, [adminKey]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      {resumo && (
        <Card>
          <SectionHead title={`Resumo do mês · ${resumo.mes}`} sub={resumo.resumo}/>
          <div style={{display:"flex",gap:S.md,flexWrap:"wrap"}}>
            {[{l:"Faturamento",v:`R$ ${Number(resumo.faturamento).toLocaleString("pt-BR")}`,c:A.green},
              {l:"Atendimentos",v:resumo.atendimentos,c:A.cyan},
              {l:"Ticket médio",v:`R$ ${Number(resumo.ticket).toLocaleString("pt-BR")}`,c:A.blue},
              {l:"Faltas",v:resumo.faltas,c:A.amber},
              {l:"Cancelados",v:resumo.cancelados,c:A.textSec}].map((st,i)=>(
          ...
          <div style={{display:"flex",gap:S.lg,flexWrap:"wrap",marginTop:S.md}}>
            <div style={{flex:1,minWidth:180}}>
              <div style={{color:A.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>Top serviços</div>
              {(resumo.topServicos||[]).map((s,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"2px 0",color:A.textSec}}><span>{s.servico}</span><span>{s.qtd}</span></div>))}
              {(!resumo.topServicos||!resumo.topServicos.length)&&<div style={{fontSize:11,color:A.textMuted}}>Sem dados no mês.</div>}
            </div>
            <div style={{flex:1,minWidth:180}}>
              <div style={{color:A.textMuted,fontSize:10,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>Top clientes</div>
              {(resumo.topClientes||[]).map((s,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"2px 0",color:A.textSec}}><span>{s.cliente}</span><span>R$ {Number(s.gasto).toLocaleString("pt-BR")}</span></div>))}
              {(!resumo.topClientes||!resumo.topClientes.length)&&<div style={{fontSize:11,color:A.textMuted}}>Sem dados no mês.</div>}
            </div>
          </div>
        </Card>
      )}
...
// ─── SAÚDE REAL (v11): só sinais verdadeiros — nada simulado ───────────────
const SaudeReal = () => {
  const [ping, setPing] = useState(undefined);   // undefined=testando · {ok,ms}
  const [agenda, setAgenda] = useState(null);    // {status,nome} | null
  const [online, setOnline] = useState(typeof navigator!=="undefined" ? navigator.onLine : true);
  const [sw, setSw] = useState(false);
  const testar = () => {
    setPing(undefined);
    api.ping().then(r=>setPing(r||{ok:false})).catch(()=>setPing({ok:false}));
    const k = adminKeyStore.get();
    if (ENV.hasBackend && k) api.calendarioGet(k).then(r=>{ if(r&&r.success) setAgenda(r); }).catch(()=>{});
  };
  useEffect(()=>{
    testar();
    const on=()=>setOnline(true), off=()=>setOnline(false);
    window.addEventListener("online",on); window.addEventListener("offline",off);
    if (typeof navigator!=="undefined" && navigator.serviceWorker) navigator.serviceWorker.getRegistration().then(r=>setSw(!!r)).catch(()=>{});
    return ()=>{ window.removeEventListener("online",on); window.removeEventListener("offline",off); };
  },[]);
  const back = !ENV.hasBackend ? {c:A.textMuted, txt:"Não configurado"}
    : ping===undefined ? {c:A.amber, txt:"Testando…"}
    : ping.ok ? {c:A.green, txt:`Conectado · ${ping.ms}ms`} : {c:A.red, txt:"Sem resposta"};
  const itens = [
    {nome:"Backend (Google Apps Script)", desc: ENV.hasBackend?"Agendamentos, clientes, caixa":"Defina VITE_GAS_URL na Vercel", c:back.c, txt:back.txt, icon:"cpu"},
    {nome:"Planilha (banco de dados)", desc:"Acessada através do backend", c:back.c, txt: ENV.hasBackend ? (ping&&ping.ok?"Operacional":"Depende do backend") : "—", icon:"database"},
    {nome:"Google Agenda", desc:"Recebe os novos agendamentos", c: agenda ? (agenda.status==="ok"?A.green:agenda.status==="invalido"?A.red:A.textMuted) : A.textMuted, txt: agenda ? (agenda.status==="ok"?`Vinculada${agenda.nome?" · "+agenda.nome:""}`:agenda.status==="invalido"?"ID inválido":"Desligada (só planilha)") : "Não verificada", icon:"calendar"},
    {nome:"WhatsApp (Meta)", desc:"Confirmações e lembretes automáticos", c:A.textMuted, txt:"Configuração no Apps Script", icon:"phone"},
    {nome:"Internet deste aparelho", desc:"Conexão do navegador", c: online?A.green:A.red, txt: online?"Online":"Offline", icon:"wifi"},
    {nome:"Aplicativo instalável (PWA)", desc:"Site instalável que abre offline", c: sw?A.green:A.textMuted, txt: sw?"Ativo":"Não registrado", icon:"activity"},
  ];
  return (
    <div style={{display:"flex",flexDirection:"column",gap:S.md,animation:`fadeUp ${M.base}`}}>
      <Card>
        <SectionHead title="Conexões" sub="Verificações reais, feitas agora neste aparelho"
          action={<Btn variant="secondary" size="sm" onClick={testar}>Testar de novo</Btn>}/>
        <div style={{display:"flex",flexDirection:"column",gap:2}}>
          {itens.map((sv,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:R.md,animation:`fadeUp ${M.base} ${i*40}ms both`}}>
              <div style={{width:28,height:28,borderRadius:R.md,background:`${sv.c}10`,border:`1px solid ${sv.c}20`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                <Ico n={sv.icon} size={12} color={sv.c}/>
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{color:A.textPri,fontSize:11.5,fontWeight:600}}>{sv.nome}</div>
                <div style={{color:A.textMuted,fontSize:9}}>{sv.desc}</div>
              </div>
              <Badge color={sv.c} dot>{sv.txt}</Badge>
            </div>
          ))}
        </div>
      </Card>
...
// ─── SISTEMA PAGE ──────────────────────────────────────────────────────────
// CENTRAL DO PROJETO — aba DEMO/template: checklist vivo (pronto · lançar · backlog), editável e
// salvo na config. Serve de modelo para projetos futuros. Só admin; nunca aparece pro cliente.
const CentralPage = () => {
  const toast = useToast();
  useStore(configStore);
  const live = useLive();
  const proj = (configStore.get() || {}).projeto || DEFAULT_CONFIG.projeto;
  const setProj = (patch) => configStore.set(c => ({ ...c, projeto: { ...(c.projeto || DEFAULT_CONFIG.projeto), ...patch } }));
  const toggle = (lista, id) => setProj({ [lista]: (proj[lista] || []).map(it => it.id === id ? { ...it, feito: !it.feito } : it) });
  const pct = (arr) => (arr && arr.length) ? Math.round(arr.filter(x => x.feito).length / arr.length * 100) : 0;
  const salvar = async () => {
    const c = configStore.get();
    const payload = { barbearia:c.barbearia, servicos:c.servicos, barbeiros:c.barbeiros||[], horarios:c.horarios, operacao:c.operacao, fidelidade:c.fidelidade, descontos:c.descontos, metas:c.metas, sorteio:c.sorteio, projeto:c.projeto, produtos:c.produtos };
    if(!ENV.hasBackend){ toast("Salvo localmente (demonstração)", A.green, "check"); return; }
    const key = window.prompt("Chave de admin (ADMIN_KEY) para salvar no servidor:");
    if(!key){ toast("Salvamento cancelado", A.textSec, "warning"); return; }
    try { const r = await api.salvarConfig(key, payload); toast(r&&r.success?"Central salva no servidor ✓":(r&&(r.error||r.erro))||"Chave recusada", r&&r.success?A.green:A.amber, r&&r.success?"check":"warning"); }
    catch(e){ toast("Falha de rede ao salvar", A.red, "warning"); }
  };
  const Checklist = ({ lista, titulo, sub, cor }) => (
    <Card>
      <SectionHead title={titulo} sub={sub}/>
      <div style={{height:6,background:A.border,borderRadius:99,marginBottom:S.md}}><div style={{height:6,width:`${pct(proj[lista])}%`,background:cor,borderRadius:99,transition:"width .3s"}}/></div>
      {(proj[lista]||[]).map(it=>(
        <label key={it.id} onClick={()=>toggle(lista,it.id)} style={{display:"flex",alignItems:"flex-start",gap:10,cursor:"pointer",padding:"6px 0"}}>
          <input type="checkbox" readOnly checked={!!it.feito} style={{width:17,height:17,marginTop:1,accentColor:cor}}/>
          <span style={{fontSize:12.5,color:it.feito?A.textMuted:A.textPri,textDecoration:it.feito?"line-through":"none"}}>{it.label}</span>
        </label>
      ))}
    </Card>
  );
  return (
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      {proj.modoDemo && (
        <Card style={{border:`1px solid ${A.amber}55`,background:`${A.amber}12`}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <span style={{fontSize:24}} aria-hidden="true">⚙️</span>
            <div>
              <div style={{fontWeight:800,color:A.amber,fontSize:14}}>VERSÃO DE CRIAÇÃO (DEMO)</div>
              <div style={{fontSize:11,color:A.textSec}}>Este painel é o modelo/template do projeto — edite os itens, marque o que concluir e clique em Salvar. Fica como base pronta para os próximos projetos.</div>
            </div>
          </div>
        </Card>
      )}
      <Card>
        <SectionHead title="Central do Projeto" sub={`Versão ${proj.versao||"—"} · ${(ENV.hasBackend&&live.mode==="real")?"backend conectado":"modo demonstração"}`}/>
        <label style={{display:"flex",alignItems:"center",gap:10,marginTop:4}}>
          <div onClick={()=>setProj({modoDemo:!proj.modoDemo})} style={{cursor:"pointer",width:46,height:26,borderRadius:R.pill,position:"relative",background:proj.modoDemo?A.amber:A.border,flexShrink:0}}>
            <div style={{position:"absolute",top:3,left:proj.modoDemo?23:3,width:20,height:20,borderRadius:"50%",background:"#fff"}}/>
          </div>
          <b style={{fontSize:13}}>Marcar como versão DEMO / template</b>
        </label>
      </Card>
      <Card>
        <SectionHead title="✅ Já pronto no sistema" sub={`${(proj.prontos||[]).length} recursos entregues`}/>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {(proj.prontos||[]).map((p,i)=><Badge key={i} color={A.green}>{p}</Badge>)}
        </div>
      </Card>
      <Checklist lista="lancamento" titulo="🚀 Para lançar" sub={`${pct(proj.lancamento)}% concluído · passos manuais seus`} cor={A.cyan}/>
      <Checklist lista="backlog" titulo="🔮 Próximas ideias (V2 / V3)" sub={`${pct(proj.backlog)}% concluído · evoluções futuras`} cor={A.purple}/>
      <div style={{display:"flex",justifyContent:"flex-end"}}>
        <Btn size="sm" onClick={salvar}><Ico n="check" size={11} color="#fff"/>Salvar no servidor</Btn>
      </div>
    </div>
  );
};
...
const SystemPage = () => {
  ...
  const tabs=[{id:"health",label:"Conexões"},{id:"integr",label:"Integrações & Dados"}];
    ...
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      <div>
        ...
        <div style={{color:A.textSec,fontSize:12,marginTop:4}}>
          Conexões do site e integrações da barbearia
        </div>
      </div>
      ...
      {/* ABA: Conexões — v11: só verificações REAIS (ping no GAS, agenda, internet, PWA) */}
      {activeTab==="health"&&<SaudeReal/>}
...
const ConfigPage = () => {
    ...
    api.getConfig().then(r => {
      ...
      if (vivo && c && typeof c === "object") {
        configStore.set(prev => ({ ...prev, ...c,
          ...
          descontos:  { ...(prev.descontos||{}),  ...(c.descontos||{}) },
          metas:      { ...(prev.metas||{}),      ...(c.metas||{}) },
          sorteio:    { ...(prev.sorteio||{}),    ...(c.sorteio||{}) },
          projeto:    { ...(prev.projeto||{}),    ...(c.projeto||{}) } }));
      }
    }).catch(()=>{});
    return () => { vivo = false; };
  }, []);
  ...
  // Salva a CONFIG RICA no backend (P1-1) — inclui agora os descontos por múltiplos serviços.
  // Reaproveitado pelos botões "Salvar" das abas Barbearia e Descontos: o que o dono edita
  // aqui CHEGA ao portal do cliente (que lê serviços/barbeiros/descontos do backend).
  const salvarConfigGAS = async () => {
    const gasConfig = {
    ...
    if(!ENV.hasBackend){ toast("Salvo localmente (modo demonstração)", A.green, "check"); return; }
    ...
    try {
      ...
      toast(r&&r.success?"Configuração salva no GAS ✓ (já vale no portal do cliente)":(r&&(r.error||r.erro))||"GAS recusou a chave admin", r&&r.success?A.green:A.amber, r&&r.success?"check":"warning");
    } catch(e){ toast("Falha de rede ao salvar no GAS", A.red, "warning"); }
  };

  // v13: metas de vendas da equipe — carrega o ranking real (faturamento × meta) do backend.
  const [metasData, setMetasData] = useState(null);
  const carregarMetas = () => { if(!ENV.hasBackend) return; api.metasPainel(adminKeyStore.get()).then(r=>{ if(r&&r.success) setMetasData(r); }).catch(()=>{}); };
  useEffect(()=>{ if(tab==="metas") carregarMetas(); }, [tab]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:S.lg}}>
      ...
      {/* METAS — metas de vendas da equipe + ranking + link público por barbeiro */}
      {tab==="metas"&&(()=>{
        const m = cfg.metas || {};
        const niveis = Array.isArray(m.niveis)?m.niveis:[];
        const setM = (patch)=>configStore.set(c=>({...c,metas:{...(c.metas||{}),...patch}}));
        const setNivel = (i,patch)=>setM({ niveis: niveis.map((n,j)=>j===i?{...n,...patch}:n) });
        const setPremio = (nome,val)=>setM({ premios:{...(m.premios||{}),[nome]:val} });
        const ativos = (cfg.barbeiros||[]).filter(b=>b&&b.ativo!==false);
        const setBarbMeta = (id,val)=>configStore.set(c=>({...c,barbeiros:(c.barbeiros||[]).map(b=>b.id===id?{...b,metaMes:Number(val)||0}:b)}));
        const rankDe = (id)=>((metasData&&metasData.barbeiros)||[]).find(x=>x.id===id)||null;
        const origin = (typeof window!=="undefined")?window.location.origin:"";
        const corNivel = (nv)=> nv==="Ouro"?A.amber : nv==="Prata"?A.cyan : nv==="Bronze"?A.blue : A.textMuted;
        return (
        <Card style={{animation:`fadeUp ${M.base}`}}>
          <SectionHead title="Metas de vendas da equipe" sub="Estimula venda: meta mensal, níveis e ranking. Cada barbeiro ganha um link próprio."/>
          <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"4px 0 12px"}}>
            <input type="checkbox" checked={m.ativo!==false} onChange={e=>setM({ativo:e.target.checked})} style={{width:18,height:18,accentColor:A.cyan}}/>
            <b style={{fontSize:13}}>Programa de metas ativo</b>
          </label>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:S.md}}>
            <Field label="Meta da loja no mês (R$)"><TextInput type="number" value={m.metaLojaMes??0} onChange={v=>setM({metaLojaMes:Number(v)||0})}/></Field>
            <Field label="Meta padrão por barbeiro (R$)"><TextInput type="number" value={m.metaBarbeiroMes??0} onChange={v=>setM({metaBarbeiroMes:Number(v)||0})}/></Field>
          </div>
          <div style={{marginTop:S.lg}}>
            <div style={{fontSize:11,letterSpacing:"0.06em",textTransform:"uppercase",color:A.textMuted,marginBottom:8}}>Níveis (% da meta) e prêmios</div>
            {niveis.map((n,i)=>(
              <div key={i} style={{display:"flex",alignItems:"flex-end",gap:10,marginBottom:8}}>
                <div style={{width:90}}><Field label="Nível"><TextInput value={n.nome} onChange={v=>setNivel(i,{nome:v})}/></Field></div>
                <div style={{width:100}}><Field label="% da meta"><TextInput type="number" value={n.pct} onChange={v=>setNivel(i,{pct:Number(v)||0})}/></Field></div>
                <div style={{flex:1}}><Field label="Prêmio (opcional)"><TextInput value={(m.premios||{})[n.nome]||""} onChange={v=>setPremio(n.nome,v)} placeholder="ex.: folga, bônus…"/></Field></div>
              </div>
            ))}
          </div>
          <div style={{marginTop:S.lg}}>
            <div style={{fontSize:11,letterSpacing:"0.06em",textTransform:"uppercase",color:A.textMuted,marginBottom:8}}>Meta e link público por barbeiro</div>
            {ativos.length===0 && <div style={{fontSize:12,color:A.textSec}}>Cadastre barbeiros na aba “Barbeiros”.</div>}
            {ativos.map(b=>{ const r=rankDe(b.id); const link=r&&r.token?`${origin}/barbeiro?b=${r.token}`:null; return (
              <div key={b.id} style={{border:`1px solid ${A.border}`,borderRadius:R.md,padding:"10px 12px",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"flex-end",gap:10}}>
                  <div style={{flex:1,fontWeight:700,fontSize:13}}>{b.nome}</div>
                  <div style={{width:150}}><Field label="Meta do mês (R$)"><TextInput type="number" value={b.metaMes??""} onChange={v=>setBarbMeta(b.id,v)} placeholder={String(m.metaBarbeiroMes||0)}/></Field></div>
                </div>
                {r && <div style={{fontSize:11,color:A.textSec,marginTop:6}}>Este mês: R$ {r.faturado} de R$ {r.meta||"—"} · {r.pct}% {r.nivel&&<b style={{color:corNivel(r.nivel)}}>· {r.nivel}</b>} · {r.atendimentos} atend.</div>}
                {link ? <div style={{display:"flex",alignItems:"center",gap:8,marginTop:6}}>
                  <input readOnly value={link} onFocus={e=>e.target.select()} style={{flex:1,fontSize:11,padding:"6px 8px",borderRadius:R.sm,border:`1px solid ${A.border}`,background:A.bg2,color:A.textSec}}/>
                  <Btn variant="secondary" size="sm" onClick={()=>{ try{navigator.clipboard.writeText(link); toast("Link copiado ✓",A.green,"check");}catch(e){toast("Selecione e copie o link",A.textSec,"copy");} }}>Copiar</Btn>
                </div> : <div style={{fontSize:10.5,color:A.textMuted,marginTop:6}}>Salve e clique em Atualizar para gerar o link público.</div>}
              </div>
            ); })}
          </div>
          {metasData && metasData.barbeiros && metasData.barbeiros.length>0 && (
            <div style={{marginTop:S.lg}}>
              <div style={{fontSize:11,letterSpacing:"0.06em",textTransform:"uppercase",color:A.textMuted,marginBottom:8}}>Ranking do mês ({metasData.mes})</div>
              {metasData.loja && metasData.loja.meta>0 && <div style={{fontSize:12,color:A.textSec,marginBottom:8}}>Loja: R$ {metasData.loja.faturado} de R$ {metasData.loja.meta} ({metasData.loja.pct}%)</div>}
              {metasData.barbeiros.map(x=>(
                <div key={x.id} style={{marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:3}}>
                    <span><b>#{x.posicao}</b> {x.barbeiro} {x.nivel&&<b style={{color:corNivel(x.nivel)}}>· {x.nivel}</b>}</span>
                    <span style={{color:A.textSec}}>R$ {x.faturado}{x.meta?` / ${x.meta}`:""} · {x.pct}%</span>
                  </div>
                  <div style={{height:6,background:A.border,borderRadius:99}}><div style={{height:6,width:`${Math.min(100,x.pct)}%`,background:A.cyan,borderRadius:99}}/></div>
                </div>
              ))}
            </div>
          )}
          <div style={{marginTop:S.md,display:"flex",gap:8,justifyContent:"flex-end"}}>
            <Btn variant="secondary" size="sm" onClick={carregarMetas}>Atualizar</Btn>
            <Btn size="sm" onClick={async()=>{ await salvarConfigGAS(); carregarMetas(); }}><Ico n="check" size={11} color="#fff"/>Salvar</Btn>
          </div>
        </Card>
        );
      })()}

      {/* DESCONTOS — desconto automático por múltiplos serviços (escada % editável) */}
      {tab==="descontos"&&(()=>{
        const d = cfg.descontos || {};
        const escada = Array.isArray(d.escada) ? d.escada : [];
        const setD = (patch)=>configStore.set(c=>({...c,descontos:{...(c.descontos||{}),...patch}}));
        const setRow = (i,patch)=>setD({ escada: escada.map((r,j)=>j===i?{...r,...patch}:r) });
        const addRow = ()=>setD({ escada:[...escada, { qtd: escada.reduce((m,r)=>Math.max(m,Number(r.qtd)||0),1)+1, pct:0 }] });
        const delRow = (i)=>setD({ escada: escada.filter((_,j)=>j!==i) });
        const pctFor = (qtd)=>{ const e=[...escada].sort((a,b)=>(Number(a.qtd)||0)-(Number(b.qtd)||0)); let p=0; e.forEach(r=>{ if(qtd>=(Number(r.qtd)||0)) p=Number(r.pct)||0; }); const teto=Number(d.teto); if(!isNaN(teto)&&teto>0) p=Math.min(p,teto); return Math.max(0,p); };
        const chk = (val,on,label,hint)=>(
          <label style={{display:"flex",alignItems:"flex-start",gap:10,cursor:"pointer",padding:"8px 0"}}>
            <input type="checkbox" checked={!!val} onChange={e=>on(e.target.checked)} style={{width:18,height:18,marginTop:2,accentColor:A.cyan}}/>
            <span><b style={{fontSize:13}}>{label}</b><span style={{display:"block",fontSize:10.5,color:A.textSec}}>{hint}</span></span>
          </label>
        );
        return (
        <Card style={{animation:`fadeUp ${M.base}`}}>
          <SectionHead title="Desconto por múltiplos serviços" sub="Recompensa quem soma mais serviços no mesmo agendamento"/>
          {chk(d.ativo!==false, v=>setD({ativo:v}), "Ativar desconto por múltiplos serviços", "Aplica automaticamente quando o cliente escolhe 2 ou mais serviços.")}
          {chk(d.mostrarCliente!==false, v=>setD({mostrarCliente:v}), "Mostrar a economia para o cliente", "Exibe “você economizou R$ X (Y%)” no portal de agendamento.")}
          {chk(d.excluirVariaveis!==false, v=>setD({excluirVariaveis:v}), "Não descontar serviços “a partir de”", "Relaxamento/Hidratação e “sob consulta” são cobrados pelo valor cheio.")}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:S.md,marginTop:S.md}}>
            <Field label="Desconto máximo (%)"><TextInput type="number" value={d.teto??20} onChange={v=>setD({teto:Number(v)||0})}/></Field>
          </div>
          <div style={{marginTop:S.lg}}>
            <div style={{fontSize:11,letterSpacing:"0.06em",textTransform:"uppercase",color:A.textMuted,marginBottom:8}}>Escada de desconto (por nº de serviços)</div>
            {escada.map((r,i)=>(
              <div key={i} style={{display:"flex",alignItems:"flex-end",gap:10,marginBottom:8}}>
                <div style={{flex:1}}><Field label="A partir de (serviços)"><TextInput type="number" value={r.qtd} onChange={v=>setRow(i,{qtd:Number(v)||0})}/></Field></div>
                <div style={{flex:1}}><Field label="Desconto (%)"><TextInput type="number" value={r.pct} onChange={v=>setRow(i,{pct:Number(v)||0})}/></Field></div>
                <Btn variant="secondary" size="sm" onClick={()=>delRow(i)}><Ico n="trash" size={12} color={A.textSec}/></Btn>
              </div>
            ))}
            <Btn variant="secondary" size="sm" onClick={addRow}><Ico n="plus" size={11} color={A.textSec}/>Adicionar faixa</Btn>
          </div>
          <div style={{marginTop:S.md,background:`${A.cyan}0A`,border:`1px solid ${A.cyan}20`,borderRadius:R.md,padding:"10px 12px",color:A.textSec,fontSize:10.5}}>
            Prévia: Corte (R$ 40) + Barba (R$ 35) = R$ 75 → com <b style={{color:A.cyan}}>{pctFor(2)}%</b> o cliente paga <b style={{color:A.cyan}}>R$ {(75*(100-pctFor(2))/100).toFixed(2)}</b>.
          </div>
          <div style={{marginTop:S.md,display:"flex",gap:8,justifyContent:"flex-end"}}>
            <Btn size="sm" onClick={salvarConfigGAS}><Ico n="check" size={11} color="#fff"/>Salvar</Btn>
          </div>
        </Card>
        );
      })()}

      {/* PRODUTOS — estoque de produtos à venda (baixa ao fechar a comanda) */}
      {tab==="produtos"&&(()=>{
        const prods = Array.isArray(cfg.produtos)?cfg.produtos:[];
        const setP = (arr)=>configStore.set(c=>({...c,produtos:arr}));
        const upd = (id,patch)=>setP(prods.map(p=>p.id===id?{...p,...patch}:p));
        const add = ()=>setP([...prods,{id:(prods.reduce((m,p)=>Math.max(m,Number(p.id)||0),0)+1),nome:"",preco:0,estoque:0,ativo:true}]);
        const del = (id)=>setP(prods.filter(p=>p.id!==id));
        return (
        <Card style={{animation:`fadeUp ${M.base}`}}>
          <SectionHead title="Produtos & Estoque" sub="Itens à venda (pomada, shampoo…). A quantidade baixa sozinha ao fechar a comanda."/>
          {prods.length===0 && <div style={{fontSize:12,color:A.textSec,marginBottom:8}}>Nenhum produto ainda — adicione abaixo.</div>}
          {prods.map(p=>(
            <div key={p.id} style={{display:"flex",alignItems:"flex-end",gap:8,marginBottom:8,opacity:p.ativo!==false?1:0.5}}>
              <div style={{flex:2}}><Field label="Produto"><TextInput value={p.nome} onChange={v=>upd(p.id,{nome:v})}/></Field></div>
              <div style={{width:90}}><Field label="Preço (R$)"><TextInput type="number" value={p.preco} onChange={v=>upd(p.id,{preco:Number(v)||0})}/></Field></div>
              <div style={{width:90}}><Field label="Estoque"><TextInput type="number" value={p.estoque} onChange={v=>upd(p.id,{estoque:Number(v)||0})}/></Field></div>
              <Btn variant="secondary" size="sm" onClick={()=>upd(p.id,{ativo:p.ativo===false})}>{p.ativo!==false?"Ativo":"Inativo"}</Btn>
              <Btn variant="secondary" size="sm" onClick={()=>del(p.id)}><Ico n="trash" size={12} color={A.textSec}/></Btn>
            </div>
          ))}
          <Btn variant="secondary" size="sm" onClick={add}><Ico n="plus" size={11} color={A.textSec}/>Adicionar produto</Btn>
          <div style={{marginTop:S.md,background:`${A.amber}10`,border:`1px solid ${A.amber}30`,borderRadius:R.md,padding:"8px 10px",fontSize:10.5,color:A.textSec}}>
            A baixa de estoque acontece quando você fecha a comanda que contém o produto (escolha o produto na comanda, no Caixa).
          </div>
          <div style={{marginTop:S.md,display:"flex",justifyContent:"flex-end"}}><Btn size="sm" onClick={salvarConfigGAS}><Ico n="check" size={11} color="#fff"/>Salvar</Btn></div>
        </Card>
        );
      })()}
...
const MobilePreview = () => {
  ...
  if (live.mode === "real") return null; // C9/D1: é uma ilustração com dados de exemplo — some no modo conectado
  return (
...
function AppInner() {
  ...
  const [maisOpen, setMaisOpen] = useState(false); // v12: aba "Mais" da barra inferior
  ...
  const topbar={
    dash:    {title:"Hoje",          subtitle:"Resumo do dia da sua barbearia"},
    agenda:  {title:"Agenda",        subtitle:`${DB.agenda.length} horário(s) hoje · ${DB.agenda.filter(x=>["confirmado","realizado"].includes(x.status)).length} confirmado(s)`},
    clients: {title:"Clientes",      subtitle:`${DB.clientes.length} cliente(s) cadastrado(s) · ${DB.clientes.filter(c=>BE.shouldFlagRisk(c)).length} precisando de atenção`},
    crm:     {title:"CRM",           subtitle:"Mensagens, campanhas e reativação de clientes"},
    finance: {title:"Financeiro",    subtitle:"Receitas, despesas e resultado do mês"},
    caixa:   {title:"Caixa",         subtitle:"Comandas, pagamentos, sangria e cupons"},
    loyalty: {title:"Fidelização",   subtitle:"Programa de pontos e níveis dos clientes"},
    waitlist:{title:"Fila de espera",subtitle:`${DB.waitlist.length} cliente(s) aguardando vaga`},
    reports: {title:"Relatórios",    subtitle:"Exportações em CSV e PDF"},
    config:  {title:"Configurações", subtitle:"Serviços, equipe, horários e dados da barbearia"},
    system:  {title:"Sistema",       subtitle:"Conexões do site e integrações"},
    central: {title:"Central do Projeto", subtitle:"Roadmap, checklist de lançamento e versão DEMO"},
  };
  ...
  const renderPage=()=>{
    switch(activeNav){
      ...
      case "central":  return <CentralPage/>;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{display:"flex",flex:1,minHeight:0}}>
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflow:"auto"}}>
            ...
            <div
              key={activeNav}
              style={{
                padding: mobile ? `${S.md}px ${S.md}px 96px` : `${S.xl}px ${S.xxl}px ${S.xxl}px`,
        ...
        {/* v12: barra inferior de 5 abas no celular (blueprint Layout_e_IA_v7 / 02_SPEC_PAINEL_ADMIN) */}
        {mobile && maisOpen && (
          <div onClick={()=>setMaisOpen(false)} style={{position:"fixed",inset:0,zIndex:480,background:"rgba(0,0,0,0.5)"}}>
            <div onClick={e=>e.stopPropagation()} style={{position:"absolute",left:0,right:0,bottom:62,background:A.bg1,borderTop:`1px solid ${A.border}`,borderRadius:"16px 16px 0 0",padding:"14px 12px 12px",boxShadow:"0 -10px 30px rgba(0,0,0,.35)"}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                {[{id:"crm",l:"CRM",i:"crm"},{id:"finance",l:"Financeiro",i:"finance"},{id:"caixa",l:"Caixa",i:"finance"},{id:"loyalty",l:"Fidelização",i:"loyalty"},
                  {id:"waitlist",l:"Fila",i:"waitlist"},{id:"reports",l:"Relatórios",i:"reports"},{id:"config",l:"Config.",i:"settings"},{id:"system",l:"Sistema",i:"cpu"},{id:"central",l:"Central",i:"reports"}].map(it=>(
                  <div key={it.id} onClick={()=>{ setActiveNav(it.id); setMaisOpen(false); }} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"10px 4px",borderRadius:R.md,
                    background:activeNav===it.id?`${A.blue}12`:A.bg2,border:`1px solid ${activeNav===it.id?A.blue+"30":A.border}`,cursor:"pointer"}}>
                    <Ico n={it.i} size={15} color={activeNav===it.id?A.cyan:A.textSec}/>
                    <span style={{fontSize:9.5,color:A.textPri,fontWeight:600}}>{it.l}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {mobile && (
          <div style={{position:"fixed",left:0,right:0,bottom:0,zIndex:470,display:"flex",alignItems:"flex-end",justifyContent:"space-around",
            background:A.bg1,borderTop:`1px solid ${A.border}`,padding:"6px 4px calc(8px + env(safe-area-inset-bottom, 0px))"}}>
            {[{id:"dash",l:"Hoje",i:"sun"},{id:"agenda",l:"Agenda",i:"calendar"}].map(it=>(
              <div key={it.id} onClick={()=>{ setActiveNav(it.id); setMaisOpen(false); }} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"4px 10px",cursor:"pointer"}}>
                <Ico n={it.i} size={17} color={activeNav===it.id?A.cyan:A.textMuted}/>
                <span style={{fontSize:9.5,fontWeight:activeNav===it.id?700:500,color:activeNav===it.id?A.textPri:A.textMuted}}>{it.l}</span>
              </div>
            ))}
            <div onClick={()=>{ setActiveNav("agenda"); setMaisOpen(false); }} aria-label="Novo agendamento"
              style={{width:46,height:46,borderRadius:"50%",background:`linear-gradient(135deg,${A.blue},${A.blueHi})`,display:"flex",alignItems:"center",justifyContent:"center",
                color:"#fff",fontSize:26,fontWeight:700,marginTop:-20,boxShadow:`0 6px 16px ${A.blue}55`,cursor:"pointer",lineHeight:1}}>+</div>
            {[{id:"clients",l:"Clientes",i:"clients"}].map(it=>(
              <div key={it.id} onClick={()=>{ setActiveNav(it.id); setMaisOpen(false); }} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"4px 10px",cursor:"pointer"}}>
                <Ico n={it.i} size={17} color={activeNav===it.id?A.cyan:A.textMuted}/>
                <span style={{fontSize:9.5,fontWeight:activeNav===it.id?700:500,color:activeNav===it.id?A.textPri:A.textMuted}}>{it.l}</span>
              </div>
            ))}
            <div onClick={()=>setMaisOpen(v=>!v)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"4px 10px",cursor:"pointer"}}>
              <span style={{fontSize:17,lineHeight:"17px",color:maisOpen||["crm","finance","caixa","loyalty","waitlist","reports","config","system"].includes(activeNav)?A.cyan:A.textMuted}}>⋯</span>
              <span style={{fontSize:9.5,fontWeight:maisOpen?700:500,color:maisOpen?A.textPri:A.textMuted}}>Mais</span>
            </div>
          </div>
        )}
      </div>
