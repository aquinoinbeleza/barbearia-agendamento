...
// ═══════════════════════════════════════════════════════════════════════════
// AQUINO SaaS · v9.1  ·  ARQUIVO DEFINITIVO CONSOLIDADO (100%+)
// Base: v9 "fonte de verdade" (__2_) — contrato real do GAS v6 preservado.
// Consolidação v9.1 (merge auditado das 4 variantes v9):
// → ONBOARDING de 1º acesso (wizard 3 passos) — grava no configStore real.
// → API client estendido: ping (testar conexão), reagendar, enviarLembrete,
//   enviarSinal — além de dashboard/morningBriefing/verificarCliente/
//   meusAgendamentos/slots/agendamento/cancelar/salvarConfig.
// → Persistência real nos botões de recorrência e sinal do painel de cliente
//   (antes só toast) — degradam p/ modo demo sem backend.
// DNA: Nubank (silêncio/confiança) · TikTok (zero-think/fluidez)
//      Instagram (leveza/onboarding) · Linear (motion/refinamento)
//
// CHANGELOG v9 (fecha 100% dos gaps das auditorias v7→v8.1):
// → ConfigPage COMPLETA (P0): serviços (CRUD preço/duração), horários de
//   funcionamento por dia, slot mínimo + antecedência + % de sinal, dados da
//   barbearia (nome, logo, endereço, telefone). Tudo persistido.
// → Export CSV/PDF REAL (P1): Blob + URL.createObjectURL para CSV; janela de
//   impressão para PDF. Funciona em Relatórios, Clientes e Financeiro.
// → Credenciais via .env (P0 segurança): VITE_GAS_URL + VITE_SITE_TOKEN
//   (contrato real do V3). ADMIN_KEY fica só no GAS (server-side). Zero
//   segredo no bundle. Ver .env.example.
// → API client GAS real: ações dashboard/morningBriefing/verificarCliente/
//   meusAgendamentos/agendamento/cancelar/salvarConfig (engenharia reversa do
//   Codigo_v6.gs). Sem backend → modo demonstração automático.
// → Persistência real (P0): store localStorage (cfg + clientes + agenda),
//   sobrevive ao reload. Botão "Restaurar dados" no Sistema.
// → Mobile App COMPLETO: 6 telas (Home, Agendar, Confirmado, Histórico,
//   Perfil, Fidelidade) — antes só 3.
// → Drawer do cliente: histórico, botão de recorrência funcional e campo
//   "Último lembrete".
//
// Herdado do v8/v8.1: ⌘K busca, drawer de notificações, toast system,
// agenda com ações reais, badges dinâmicos, score bar + sparkline.
//
// Filosofia: a complexidade existe. O usuário nunca pode senti-la.
// ═══════════════════════════════════════════════════════════════════════════
...
// ─── ENV · CREDENCIAIS (P0 SEGURANÇA) ─────────────────────────────────────
// Nenhum segredo fica hardcoded no bundle. Tudo vem de import.meta.env.
...
// Contrato REAL do V3 hospedado (engenharia reversa do Codigo_v6.gs + App_v6.jsx):
//   VITE_GAS_URL    → endpoint /exec do Google Apps Script
//   VITE_SITE_TOKEN → token de origem site↔GAS (default "aq2025site")
// A senha de admin (ADMIN_KEY) vive SÓ no GAS (Script Properties) e é validada
// server-side via action "dashboard"/"validarSenha" — nunca trafega no front.
const ENV = {
...
const exportPDF = (titulo, headers, rows) => {
  ...
  try {
    ...
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${titulo}</title>
      ...
      <h1>${titulo}</h1>
      <div class="sub">AQUINO SaaS v9 · gerado em ${new Date().toLocaleString("pt-BR")}</div>
