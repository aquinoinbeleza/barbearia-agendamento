/**
 * ════════════════════════════════════════════════════════════════════════
 *  AQUINO BARBER SYSTEM · Backend Google Apps Script
 *  v9.2 — ALINHADO À ESPECIFICAÇÃO MASTER (AQUINO_100PCT_COMPLETO_v3_AUDITADO)
 *
 *  Conforma ao contrato canônico do documento master:
 *   • Schemas EXATOS: Clientes / Agendamentos / Financeiro (PARTE 6.9/6.10)
 *   • Respostas canônicas: {success,error} e {encontrado,...}
 *   • IDs: ClienteID CLI-001 sequencial · Agendamento UUID 8 hex
 *   • Status: confirmado | presenca_confirmada | cancelado | faltou | realizado
 *   • Segurança P0/P1: SITE_TOKEN, ADMIN_KEY server-side, sanitização
 *     anti-fórmula, rate limit 15/min, HMAC-SHA256 webhook, erros genéricos
 *   • Script Properties canônicas (PARTE 6.13)
...
// ─── SCORE DE CONFIANÇA — modelo REAL de produção (Codigo_v6.gs) ────────────
// Base 10 (começa confiável e DIMINUI). Penaliza ausência e cancelamentos,
// bonifica fidelidade. Alinhado ao backend hospedado v6 (fonte de verdade real).
